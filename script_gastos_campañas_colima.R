...
gastos_g_locales <- read.csv(file = "https://raw.githubusercontent.com/Zatara13/gasto_campanas_colima_g/main/PELO_20-21_CAMPA%C3%91A_G_RUBRO_210426.csv")

## Datos abiertos candidaturas federales
gastos_g_federales <- read.csv(file = "https://raw.githubusercontent.com/Zatara13/gasto_campanas_colima_g/main/PEFO_20-21_CAMPA%C3%91A_G_RUBRO_210426.csv")
...
siglas_2 <- data.frame(siglas = as.factor(c("MORENA",
                                            ...
                                            "PRI-PAN-PRD")),
                       siglasb = as.factor(c("JUNTOS HAREMOS HISTORIA",
                                             "MOVIMIENTO CIUDADANO",
                                             "PT",
                                             "PVEM",
                                             "PRI-PAN-PRD",
...
g_partidos_sm <- g_partidos %>% 
  ...
  mutate(gasto_años = gasto_dias / 365) %>% 
...
ggplot(g_partidos_sm, ## Llamamos base de datos
  ...
  scale_fill_manual(values = c("royalblue2" = "royalblue2",
                               ...
                               "green" = "green"))+
  labs(title = "Gastos en las campañas 2021 en Colima", ## Ponemos etiquetas a cosas
       subtitle = "Años que un trabajador con salario mínimo tardaría en reunir el dinero que han gastado los partidos en las campañas 2021 en Colima",
       y = "Años",
       caption = "Fuente: Datos abiertos de fiscalización del INE.
       Nota: La fecha de corte de los datos de la gráfica es 26/04/2021.
         Créditos: Página que te avisa cuanto gastan los políticos en campaña"
