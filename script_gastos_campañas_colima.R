...
gastos_g_locales <- read.csv(file = "https://raw.githubusercontent.com/Zatara13/gasto_campanas_colima_g/main/PELO_20-21_CAMPA%C3%91A_G_RUBRO_210510.csv")

## Datos abiertos candidaturas federales
gastos_g_federales <- read.csv(file = "https://raw.githubusercontent.com/Zatara13/gasto_campanas_colima_g/main/PEFO_20-21_CAMPA%C3%91A_G_RUBRO_210510.csv")
...
ggplot(g_partidos_sm, ## Llamamos base de datos
  ...
  geom_hline(yintercept = 120,
             ...
             color = "red")+
  labs(title = "Gastos en las campañas 2021 en Colima", ## Ponemos etiquetas a cosas
       subtitle = "Años que un trabajador con salario mínimo tardaría en reunir el dinero que los partidos gastaron en campaña hasta el 10/03/21",
       y = "Años",
       caption = "Fuente: Datos abiertos de fiscalización del INE.
...
## Mapas
## Generamos las bases de datos con las que trabajaremos - diputaciones federales por partidos, locales por partidos y alcaldías por partidos
f_gastos_niveles <- function(cargo_f){
  gastos_c %>% 
    ungroup() %>%
    filter(cargo == cargo_f) %>% 
    group_by(subnivel,
             siglas) %>% 
    summarise(gastos = sum(gasto))
}
...
gastos_df <- f_gastos_niveles("DIPUTACION FEDERAL MR")
gastos_dl <- f_gastos_niveles("DIPUTADO LOCAL MR")
gastos_m <- f_gastos_niveles("PRESIDENTE MUNICIPAL")

gastos_m_clave <- data.frame(subnivel = c("Municipio 1-COLIMA",
                                           "Municipio 10-TECOMAN",
                                           "Municipio 2-COMALA",
                                           "Municipio 3-COQUIMATLAN",
                                           "Municipio 4-CUAUHTEMOC",
                              ...
                              nombre = c("COLIMA",
                                         ...
                                         "MANZANILLO",
                                         "MINATITLAN"))
gastos_m <- merge(gastos_m,
                  gastos_m_clave,
                  by = "subnivel")

gastos_dl_clave <- data.frame(subnivel = c("Distrito 1-COLIMA",
                                           "Distrito 14-MINATITLAN",
                                           "Distrito 10-TECOMAN",
                                           "Distrito 11-MANZANILLO",
                                           "Distrito 12-MANZANILLO",
...
## Cargamos los mapas
setwd("C://Users//jorge//Documents//Columnas//gasto campañas Colima")

## No sé exactamente que hice pero funciona, no le muevas
dl_mapas <- st_read("Mapas/DISTRITO_LOCAL.shp")
dl_mapas_colima <- dl_mapas %>% 
  filter(ENTIDAD == 6)
rm(dl_mapas)

dl_mapas_colima <- dl_mapas_colima %>% 
  left_join(gastos_dl,
            by = "DISTRITO_L")

municipio_mapas <- st_read("Mapas/MUNICIPIO.shp")
mun_mapas_colima <- municipio_mapas %>% 
  filter(entidad == 6)
rm(municipio_mapas)

mun_mapas_colima <- mun_mapas_colima %>% 
  left_join(gastos_m,
            by = "nombre")

## Funciones para hacer la graficación
f_mapas_m <- function(partido,
                      subtitulo,
                      color){
  mun_mapas_colima %>% 
    filter(siglas == partido) %>% 
    mutate(gastos = gastos / 1000) %>% 
    ggplot(aes(fill = gastos)) +
    geom_sf()+
    geom_sf_text(aes(label = nombre),
                 size = 5)+
    coord_sf(xlim = c(-300000.0, -150000))+ ## NO MOVER ESTOS PARÁMETROS
    theme_void()+
    theme(text = element_text(size=20),
          plot.title = element_text(hjust = 0.5),
          axis.title.x =  element_blank()) +
    scale_fill_gradient("Gastos",
                        high = color,
                        low = "white")+
    labs(title = "Gastos en las campañas 2021 en Colima",
         subtitle = subtitulo,
         caption = "Fuente: Datos abiertos de fiscalización del INE.
       Los gastos se ven en miles de pesos
       Si un municipio no aparece, es porque no hay información disponible.
       Créditos: Página que te avisa cuanto gastan los políticos en campaña")
}


f_mapas_dl <- function(partido,
                       subtitulo,
                       color){
  dl_mapas_colima %>% 
    filter(siglas == partido) %>% 
    ...
    ggplot(aes(fill = gastos)) +
    geom_sf()+
    geom_sf_text(aes(label = DISTRITO_L),
                 size = 6)+
    coord_sf(xlim = c(-300000.0, -150000))+ ## NO MOVER ESTOS PARÁMETROS
    theme_void()+
    theme(text = element_text(size=20),
    ...
    labs(title = "Gastos en las campañas 2021 en Colima",
         ...
         caption = "Fuente: Datos abiertos de fiscalización del INE.
       Los gastos se ven en miles de pesos
       Si un distrito no aparece, es porque no hay información disponible.
       Créditos: Página que te avisa cuanto gastan los políticos en campaña")
}


## Gastos por distritos electorales
## Morena
f_mapas_dl("MORENA",
           "Gasto realizado en cada distrito electoral local por Morena",
           "darkred")

## Movimiento Ciudadano
f_mapas_dl("MOVIMIENTO CIUDADANO",
           "Gasto realizado en cada distrito electoral local por Movimiento Ciudadano",
           "darkorange")

## PT
f_mapas_dl("PT",
           "Gasto realizado en cada distrito electoral local por el PT",
           "red")

## PES
f_mapas_dl("PES",
           "Gasto realizado en cada distrito electoral local por el PES",
           "orchid")

## Redes Sociales Progresistas
f_mapas_dl("RSPPPN",
           "Gasto realizado en cada distrito electoral local por Redes Sociales Progresistas",
           "slategray")

## Partido Verde
f_mapas_dl("PVEM",
           "Gasto realizado en cada distrito electoral local por el Partido Verde",
           "green")

## Fuerza por México
f_mapas_dl("FS X MÉXICO",
           "Gasto realizado en cada distrito electoral local por Fuerza por México",
           "violetred")


## Nueva Alianza Colima 
f_mapas_dl("NUEVA ALIANZA COLIMA",
           "Gasto realizado en cada distrito electoral local por Nueva Alianza",
           "turquoise")


## El PRIAN
f_mapas_dl("PRI-PAN-PRD",
           "Gasto realizado en cada distrito electoral local por el PRIANRD",
           "royalblue")


## Gastos por municipios
## Morena
f_mapas_m("MORENA",
           "Gasto realizado en cada municipio por Morena",
          "darkred")

## Movimiento Ciudadano
f_mapas_m("MOVIMIENTO CIUDADANO",
           "Gasto realizado en cada municipio por Movimiento Ciudadano",
          "darkorange")

## PT
f_mapas_m("PT",
           "Gasto realizado en cada municipio por el PT",
          "red")

## PES
f_mapas_m("PES",
           "Gasto realizado en cada municipio por el PES",
          "orchid")

## Redes Sociales Progresistas
f_mapas_m("RSPPPN",
           "Gasto realizado en cada municipio por Redes Sociales Progresistas",
          "slategray")

## Partido Verde
f_mapas_m("PVEM",
           "Gasto realizado en cada municipio por el Partido Verde",
          "green")

## Fuerza por México
f_mapas_m("FS X MÉXICO",
           "Gasto realizado en cada municipio por Fuerza por México",
          "violetred")


## Nueva Alianza Colima 
f_mapas_m("NUEVA ALIANZA COLIMA",
           "Gasto realizado en cada municipio por Nueva Alianza",
          "turquoise")


## El PRIAN
f_mapas_m("PRI-PAN-PRD",
           "Gasto realizado en cada municipio por el PRIANRD",
          "royalblue")