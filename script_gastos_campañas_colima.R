...
gastos_g_locales <- read.csv(file = "https://raw.githubusercontent.com/Zatara13/gasto_campanas_colima_g/main/PELO_20-21_CAMPA%C3%91A_G_RUBRO.csv")

## Datos abiertos candidaturas federales
gastos_g_federales <- read.csv(file = "https://raw.githubusercontent.com/Zatara13/gasto_campanas_colima_g/main/PEFO_20-21_CAMPA%C3%91A_G_RUBRO.csv")
...
f_graficado <- function(df,
  ...
  ggplot(df %>% 
    ...
    labs(title = "Gastos en las campañas 2021 en Colima",
       ...
       Nota: La fecha de corte de los datos de la gráfica es 17/05/2021.
...
ggplot(g_partidos_sm, ## Llamamos base de datos
  ...
  geom_hline(yintercept = 120,
             ...
             color = "red")+
  labs(title = "Gastos en las campañas 2021 en Colima", ## Ponemos etiquetas a cosas
       subtitle = "Años que un trabajador con salario mínimo tardaría en reunir el dinero que los partidos gastaron en campaña hasta el 17/03/21",
       y = "Años",
       caption = "Fuente: Datos abiertos de fiscalización del INE.
